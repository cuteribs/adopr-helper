...
export function activate(context: any) {
  ...
  const setPatCommand = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      console.debug(
        vscode.workspace.getConfiguration().get<string>("adopr-helper.pat")
      );
    ...
    async () => {
      ...
      if (!pat) {
        vscode.window.showErrorMessage(
          "Azure DevOps PAT not set. Please run 'Set Azure DevOps PAT' first."
        );
        return;
      }
      ...
      const prDetailsUrl = `https://dev.azure.com/${organization}/${project}/_apis/git/repositories/${repository}/pullRequests/${pullRequestId}?api-version=${adoApiVersion}`;

      try {
        const prDetailsResponse = await fetch(prDetailsUrl, {
          headers: headers,
        });
        if (!prDetailsResponse.ok) {
          throw new Error(
            `HTTP ${prDetailsResponse.status}: ${prDetailsResponse.statusText}`
          );
        }
        ...
        const sourceBranch = prDetails.sourceRefName?.replace(
          "refs/heads/",
          ""
        );
        const targetBranch = prDetails.targetRefName?.replace(
          "refs/heads/",
          ""
        );
        ...
        if (!sourceBranch || !targetBranch) {
          vscode.window.showErrorMessage(
            "Could not determine source or target branch from PR details."
          );
          return;
        }

        // 2. Use /diffs/commits endpoint
        const diffsUrl = `https://dev.azure.com/${organization}/${project}/_apis/git/repositories/${repository}/diffs/commits?baseVersion=${encodeURIComponent(
          targetBranch
        )}&targetVersion=${encodeURIComponent(
          sourceBranch
        )}&$top=2000&api-version=${adoApiVersion}`;

        const diffsResponse = await fetch(diffsUrl, { headers: headers });

        if (!diffsResponse.ok) {
          throw new Error(
            `HTTP ${diffsResponse.status}: ${diffsResponse.statusText}`
          );
        }
        ...
        const isSupportedExtension = (diff: any) => {
          return (
            diff.path &&
            !diff.isFolder &&
            diff.url &&
            supportedExtensions.some((ext) =>
              diff.path.toLowerCase().endsWith(ext)
            )
          );
        };

        const fileItems = changes
          .filter((c: any) => isSupportedExtension(c.item))
          .map((c: any) => ({ path: c.item.path, url: c.item.url }));
        ...
        const downloadFolder = vscode.Uri.joinPath(
        ...
        for (const file of fileItems) {
          try {
            ...
            // Use Accept: text/plain for file download
            const fileHeaders = { ...headers, Accept: 'text/plain' };
            const fileResponse = await fetch(file.url, { headers: fileHeaders });

            if (!fileResponse.ok) {
              vscode.window.showWarningMessage(
                `Failed to download ${file.path}: HTTP ${fileResponse.status}`
              );
              continue;
            }
            ...
            await vscode.workspace.fs.createDirectory(
              fullFilePath.with({
                path: fullFilePath.path.substring(
                  0,
                  fullFilePath.path.lastIndexOf("/")
                ),
              })
            );
            ...
            vscode.window.showWarningMessage(
              `Error downloading ${file.path}: ${err.message}`
            );
          }
        }
        vscode.window.showInformationMessage(
          `Downloaded ${fileItems.length} files to ${downloadFolder.fsPath}`
        );
      } catch (err: any) {
        console.error(err);
        vscode.window.showErrorMessage(
          `Failed to fetch PR changes: ${err.message}`
        );
      }
    }
  );
